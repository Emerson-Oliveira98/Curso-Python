...
# parte 2 implementar o algoritmo de busca de binário
...
# parte 3 implementar função que faz a verificação do CPF, o dedup devolve o resultado esperado
def criar_lista_dedup_ordenada(lista):
    lista = [str(cpf).replace('.', '').replace('-', '') for cpf in lista]
    lista = [cpf for cpf in lista if len(cpf)]
